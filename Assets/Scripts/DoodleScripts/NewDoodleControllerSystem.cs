...
public class NewDoodleControllerSystem : MonoBehaviour
{
    ...
    {
        if (other.collider.CompareTag("Fanar") && _rb.velocity == new Vector2(_rb.velocity.x, 0))
        {
            _isOnSpring = true;
        }
        if (other.collider.CompareTag("Jetpack") && _rb.velocity == new Vector2(_rb.velocity.x, 0))
        {
            _isOnJetpack = true;
        }
        else if (other.collider.CompareTag("normalPlatform") && _rb.velocity == new Vector2(_rb.velocity.x, 0) ||
                other.collider.CompareTag("moverPlatform") && _rb.velocity == new Vector2(_rb.velocity.x, 0))
        {
            _isOnGrounded = true;
        }
    }
    ...
    private void DoodleMoveUpdateSystem()
    {
        ...
        _rb.velocity = new Vector2(_moveDirection * _moveSpeed * _doodleHorizontalSpeed * Time.deltaTime, _rb.velocity.y);
    }
